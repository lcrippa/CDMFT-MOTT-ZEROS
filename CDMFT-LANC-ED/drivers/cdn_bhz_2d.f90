program cdn_bhz_2d
  ...
  logical                                         :: converged
  real(8)                                         :: ts,Mh,lambda,wmixing,observable,TEST_RE_PART
  !Bath:
  ...
  call parse_input_variable(TEST_RE_PART,"TEST_RE_PART",finput,default=0.d0,comment="crystal field splitting")
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     Smats(1,1,1,1,1,1,:)=Smats(1,1,1,1,1,1,:)+TEST_RE_PART
     Smats(1,1,1,1,2,2,:)=Smats(1,1,1,1,2,2,:)+TEST_RE_PART
     Smats(1,1,Nspin,Nspin,1,1,:)=Smats(1,1,Nspin,Nspin,1,1,:)+TEST_RE_PART
     Smats(1,1,Nspin,Nspin,2,2,:)=Smats(1,1,Nspin,Nspin,2,2,:)+TEST_RE_PART
     Smats(2,2,1,1,1,1,:)=Smats(2,2,1,1,1,1,:)+TEST_RE_PART
     Smats(2,2,1,1,2,2,:)=Smats(2,2,1,1,2,2,:)+TEST_RE_PART
     Smats(2,2,Nspin,Nspin,1,1,:)=Smats(2,2,Nspin,Nspin,1,1,:)+TEST_RE_PART
     Smats(2,2,Nspin,Nspin,2,2,:)=Smats(2,2,Nspin,Nspin,2,2,:)+TEST_RE_PART
